...
void UnPoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  max_idx_.Reshape(bottom[0]->num(), channels_, unpooled_height_,
      unpooled_width_);
  }
}
  ...
  case PoolingParameter_PoolMethod_MAX:
    ...
    for (int c = 0; c < channels_; ++c) {
	  ...
	  int hstart = ph * stride_h_ - pad_h_;
	      ...
	      const int unpool_index = h * unpooled_width_ + w;
	      top_data[unpool_index] = bottom_data[index];
	      if (use_top_mask) {
		top_mask[unpool_index] = static_cast<Dtype>(index);
	      } else {
	  ...
	  if (use_top_mask) {
	    top_mask[static_cast<int>(mask_data[index])] = static_cast<Dtype>(index);
	  } else {
	    mask[static_cast<int>(mask_data[index])] = index;
	  }
        }
      }
